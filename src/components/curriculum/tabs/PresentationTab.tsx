...
export const PresentationTab: React.FC<PresentationTabProps> = ({ course }) => {
  ...
  const [currentSlideIndex, setCurrentSlideIndexState] = useState<number>(() =>
    curriculumNavStore.getSlideForCourse(course.id, 0)
  );

  const setCurrentSlideIndex = (indexOrFn: number | ((prev: number) => number)) => {
    setCurrentSlideIndexState(prev => {
      const next = typeof indexOrFn === 'function' ? indexOrFn(prev) : indexOrFn;
      curriculumNavStore.setSlideForCourse(course.id, next);
      return next;
    });
  };
