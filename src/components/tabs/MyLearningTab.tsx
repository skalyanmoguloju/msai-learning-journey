...
import { getWeekComponent, isWeekAvailable } from '../../curriculum/registry';
...
export const MyLearningTab: React.FC<MyLearningTabProps> = ({
  ...
  // Active Week Navigation State (defaults to first module / Week 01)
  const [activeWeekId, setActiveWeekId] = useState<string>(
    course.modules[0]?.id || 'week-01'
  );

  // Sync active week when course changes
  useEffect(() => {
    if (course.modules.length > 0) {
      setActiveWeekId(course.modules[0].id);
    }
  }, [course.id]);
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
          {course.modules.map((mod) => {
            ...
            return (
              <button
                key={mod.id}
                onClick={() => setActiveWeekId(mod.id)}
